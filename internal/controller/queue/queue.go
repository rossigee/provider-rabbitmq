...
const errNotQueue = "managed resource is not a Queue custom resource"

func Setup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1beta1.QueueKind)
	...
	return ctrl.NewControllerManagedBy(mgr).Named(name).WithOptions(o.ForControllerRuntime()).
		WithEventFilter(resource.DesiredStateChanged()).For(&v1beta1.Queue{}).Complete(r)
}
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	queue, err := c.service.GetQueue(ctx, cr.Spec.ForProvider.Name, cr.Spec.ForProvider.VHost)
	if err != nil {
...
func (c *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	if err != nil {
		return managed.ExternalCreation{}, errors.Wrap(err, "failed to create queue")
	}
...
func (c *external) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	...
	err := c.service.DeleteQueue(ctx, cr.Spec.ForProvider.Name, cr.Spec.ForProvider.VHost)
	if err != nil && !clients.IsNotFound(err) {
		return managed.ExternalDelete{}, errors.Wrap(err, "failed to delete queue")
	}
